...
public sealed class RegisteredServicesContentViewModelTransformerTests
{
    ...
    public void TransformContentToViewModel_TViewModel__ThrowsInvalidOperationException__WhenTransformHandlerIsNotRegistered()
    {
        ...
        var act = () => {
            _ = registeredServicesContentViewModelTransformer.TransformContentToViewModel<CardViewModel>(fakeCardContent);
        };

        var exception = Assert.ThrowsException<InvalidOperationException>(act);
        Assert.AreEqual($"Cannot resolve transform handler for `{typeof(ExternalResourceContent)}` to `{typeof(CardViewModel)}`.", exception.Message);
    }

    [TestMethod]
    public void TransformContentToViewModel_TViewModel__InvokesExpectedTransformHandler()
    {
        var mockTransformHandler = new Mock<IContentViewModelTransformHandler<ExternalResourceContent, CardViewModel>>();

        var serviceProvider = new Mock<IServiceProvider>();
        var expectedServiceType = typeof(IContentViewModelTransformHandler<ExternalResourceContent, CardViewModel>);
